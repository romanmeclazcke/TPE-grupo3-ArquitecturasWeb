...
public class UsuarioController {
    ...
    @PostMapping("")
    public ResponseEntity<?> crearUsuario(@Parameter(description = "DTO de la cuenta a crear", required = true, schema = @Schema(implementation = CuentaRequestDto.class)) @RequestBody @Valid UsuarioRequestDto UsuarioRequestDto){
        try {
    ...
    @Operation(
            ...
            description = "Endpoint para obtener al usuario que le pertener un email"
    )
    @ApiResponses(
            value = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Se obtuvo la cuenta asociada al email",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = UsuarioResponseDto.class)
                            )
                    ...
                    @ApiResponse(
                            responseCode = "404",
                            description = "El mail no pertenece a un usuario"
                    )
            }
    )
    @GetMapping("/email/{userEmail}")
    public ResponseEntity<?> getUserByEmail(@Parameter(description = "Email a buscar", example = "carlitos@gmail.com" , required = true) @PathVariable String userEmail) {
        try {
