...
public class Factura_ProductoDAO {
    ...
    public ProductoDTO getProductoMayorRecaudacion() throws IOException {
        ...
        try {
            PreparedStatement ps = conn.prepareStatement("SELECT id_producto, nombre, SUM(cantidad*valor) AS recaudacion FROM producto p JOIN factura_producto fp USING (id_producto)" +
                                                                "GROUP BY id_producto, nombre ORDER BY recaudacion DESC LIMIT 1");
